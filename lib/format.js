...
var Format = function(options) {
    if (typeof options === 'string') {
        this.filename = path.join(config.directory, options);
...
Format.prototype.supported = function(callback) {
    var supported = this.formats.hasOwnProperty(this.format);
...
Format.prototype.remove = function(rowId, callback) {
    this.formats[this.format].remove(this.filename, rowId, function(err, status) {
        if (err) callback(err);
        else callback(null, status);
    });
};
