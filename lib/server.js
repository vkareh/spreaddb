...
app.post('/:filename', function(req, res) {
    ...
    req.on('end', function(err) {
        if (err) res.send(new Error('There was an error parsing your data'));
            ...
            var row = new Row({spreadsheet: req.params.filename, data: data});
...
app.put('/:filename/:rowId', function(req, res) {
    ...
    req.on('end', function(err) {
        if (err) res.send(new Error('There was an error parsing your data'));
            ...
            var row = new Row({spreadsheet: req.params.filename, _id: req.params.rowId, data: data});
...
app.delete('/:filename/:rowId', function(req, res) {
    var row = new Row({spreadsheet: req.params.filename, _id: req.paras.rowId});
